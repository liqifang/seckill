...
public class UserServiceImpl implements UserService {
    ...
    public Response<LoginUserRspVO> login(LoginUserReqVO loginUserReqVO) {
        ...
        if (Objects.equals(type, LoginTypeEnum.PASSWORD.getCode())) {
            ...
            // 密码登录：校验密码是否正确
            checkPassword(loginUserReqVO.getPassword(), userDO.getPassword(), userDO.getMobile());
        } else {
            // 验证码登录：校验验证码是否正确
    ...
    private void checkVerifyCode(String verifyCode, String mobile, String purpose) {
        ...
        // 执行 Lua 脚本：原子性的比对验证码并删除（匹配返回 1；不匹配或Key不存在返回 0）
        Long res = stringRedisTemplate.execute(checkAndDelVerifyCodeScript,
        ...
        // 走到这里说明 res == 1 ,验证码比对成功，并且验证码已被删除
    }
